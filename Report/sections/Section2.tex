...
\backgroundfigure{background_planck_cosmology.pdf}
  ...
  {The CMB and the standard cosmological model. The Planck map records small
   temperature differences in ancient light; it is not a direct map of
   present-day dark matter. The composition panel shows approximate
   present-day energy fractions inferred within $\Lambda$CDM
   \citep{2020A&A...641A...1P,2020A&A...641A...6P}.}
...
\backgroundfigure{background_s8_surveys.pdf}
  ...
  {Cosmic-shear constraints in the $\Omega_{\rm m}$--$S_8$ plane from
   successive KiDS, DES, and HSC analyses, with the chosen Planck comparison
   identified. Contours must retain their stated model and probability
   levels. Earlier releases overlap with later ones and are not independent
   experiments. The separate Euclid, Rubin, and Roman panel describes future
   scientific reach, not measured constraints. Sources are the survey papers
   cited in Section~\ref{sec:surveys-background}.}
...
When we correlate pairs of shapes, this sum produces several contributions.
Suppressing the angular and component labels, the signal is
\[
...
GG is the correlation of two lensing shears. II is the correlation of two
intrinsic shapes. GI and IG are the cross terms, with the ordering telling
us which member contributes an intrinsic shape. They are often grouped and
called GI, giving the shorthand GG + GI + II. A foreground galaxy can align
with matter that also lenses a more distant galaxy, so GI does not require
the two source galaxies to be neighbours. Random shapes and measurement
noise also contribute uncertainty, treated separately from this signal.
...
A common starting point is the \emph{nonlinear alignment} (NLA) model. It
relates alignment to the tidal field and uses a nonlinear matter power
spectrum to describe two-point correlations. Its few adjustable parameters
make it convenient, but its allowed responses are restricted
\citep{2007NJPh....9..444B}. Despite its name, it is not a complete theory of
nonlinear galaxy formation. The \emph{tidal alignment and tidal torquing}
(TATT) model adds terms describing further tidal responses and how galaxies
sample the density field. It can describe more possibilities, usually with
more adjustable parameters \citep{2019PhRvD.100j3506B}.

Neither simplicity nor complexity guarantees a better answer. A model that
is too restricted can mistake part of the alignment signal for a change in
cosmology.
A more flexible model can be harder to constrain and more costly to explore.
For example, changing alignment strength can imitate part of a change in
matter clustering. Such overlapping effects are called \emph{parameter
degeneracies}. Uncertainty in galaxy redshifts can introduce further
overlap. The DES Year 6 comparison shows that the chosen alignment model
can affect the inferred $S_8$ \citep{2026arXiv260210065D}.

Two effects help explain these difficulties. Lensing combines matter along the line of
sight, which can make distinct scale and redshift dependences look similar.
In the statistical analysis, astronomers also average over uncertain model
parameters to obtain constraints on cosmology. Giving those parameters more
freedom can change the resulting constraints. Simply replacing them with
new coordinates does not remove either issue.

Our project focuses on one part of this problem, shown in
Figure~\ref{fig:background-model-tradeoff}. We build a flexible positive
response $\mathcal A_\Theta(k,z)$ within an NLA-inspired prescription and
ask whether PCA or an autoencoder can describe its variations compactly.
This response is not the matter power spectrum itself, nor does it reproduce
all the physical terms of TATT. The original 13 parameters define the family;
the shorter representation approximates its surfaces. Any eventual use in
cosmology would still need tests of errors in lensing predictions and a
justified way to assign probabilities to the new coordinates.
Section~\ref{sec:model} now defines the model and the dataset used to test
this first step.
...
\backgroundfigure{background_model_tradeoff.pdf}
  ...
  {The modelling tradeoff motivating this project. Restricted models are
   ...
   \citep{2007NJPh....9..444B,2019PhRvD.100j3506B}.}
  {fig:background-model-tradeoff}

% Keep the background's figures before Section 3 without forcing a new page.
\FloatBarrier
