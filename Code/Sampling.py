...
def generate_component_samples(parameter_samples, z, k, *, z_star=Z_STAR):
    ...
    if z.ndim != 1 or k.ndim != 1:
        raise ValueError("z and k must be one-dimensional arrays.")
...
# -----------------------------------------------------------------------------
# 7. Reproducible ML splits and versioned HDF5 serialization
# -----------------------------------------------------------------------------
...
def build_dataset_split_indices(
    ...
    *,
    ...
    """Return reproducible, mutually exclusive train/validation/test indices."""
    number_of_models = int(number_of_models)
    ...
    rng = numpy.random.default_rng(random_seed)
    permutation = rng.permutation(number_of_models)
    train_end = int(number_of_models * train_fraction)
    validation_end = train_end + int(number_of_models * validation_fraction)
    split_indices = {
        "train": numpy.sort(permutation[:train_end]),
        "validation": numpy.sort(permutation[train_end:validation_end]),
        "test": numpy.sort(permutation[validation_end:]),
    }

    return _validate_dataset_split_indices(split_indices, number_of_models)


def _validate_dataset_split_indices(split_indices, number_of_models):
    """Validate and normalize a complete train/validation/test partition."""
    required_split_names = ("train", "validation", "test")
    if set(split_indices) != set(required_split_names):
        raise ValueError(
            f"split_indices must contain exactly {list(required_split_names)}."
        )

    normalized_indices = {}
    for split_name in required_split_names:
        indices = numpy.asarray(split_indices[split_name], dtype=numpy.int64)
        if indices.ndim != 1 or len(indices) == 0:
            raise ValueError(f"The {split_name} split must be a non-empty vector.")
        if numpy.any(indices < 0) or numpy.any(indices >= number_of_models):
            raise ValueError(f"The {split_name} split contains an invalid index.")
        normalized_indices[split_name] = numpy.sort(indices)

    combined_indices = numpy.concatenate(
        [normalized_indices[name] for name in required_split_names]
    )
    if len(combined_indices) != number_of_models or not numpy.array_equal(
        numpy.sort(combined_indices), numpy.arange(number_of_models)
    ):
...
def save_nuisance_dataset(
    ...
    *,
    ...
    """Save one fully validated, self-describing shape dataset to HDF5."""
    parameter_samples = numpy.asarray(parameter_samples)
    z = numpy.asarray(z, dtype=float)
    k = numpy.asarray(k, dtype=float)
    ...
    valid_mask = numpy.ones(len(parameter_samples), dtype=bool)
    validate_generated_dataset(
        parameter_samples,
        component_samples,
        valid_mask,
    ...
    if split_indices is not None:
        split_indices = _validate_dataset_split_indices(
            split_indices,
            len(parameter_samples),
        )

    prior_json = json.dumps(prior_specification)
    ...
    with h5py.File(output_file, "w") as dataset:
        dataset.attrs["schema_version"] = "4.0"
        ...
        dataset.attrs["shape_parameter_names"] = json.dumps(
            list(NLAModel.SHAPE_PARAMETER_NAMES)
        )
        dataset.attrs["A_theta_definition"] = "R_z * R_L * S_k_z"
        dataset.attrs["A_IA_definition"] = "-A0 * A_omega * A_theta"
        dataset.attrs["prior_specification"] = prior_json
        dataset.attrs["metadata"] = metadata_json
        ...
        for component_name in ("R_z", "R_L", "S_k_z", "A_theta"):
            components_group.create_dataset(
                component_name,
                data=numpy.asarray(component_samples[component_name]).astype(
                    storage_dtype
                ),
                **compression_options,
            )
...
def save_nuisance_dataset_in_batches(
    ...
    *,
    ...
    with h5py.File(partial_output_file, "w") as dataset:
        ...
        diagnostics_group = dataset.create_group("diagnostics")
        diagnostic_datasets = {
            "minimum_A_theta": diagnostics_group.create_dataset(
                "minimum_A_theta", shape=(number_of_models,), dtype=storage_dtype
            ),
            "maximum_A_theta": diagnostics_group.create_dataset(
                "maximum_A_theta", shape=(number_of_models,), dtype=storage_dtype
            ),
        ...
        for batch_start in range(0, number_of_models, batch_size):
            ...
            for diagnostic_name in diagnostic_datasets:  # noqa: PLC0206
                diagnostic_datasets[diagnostic_name][batch_slice] = (
                    batch_diagnostics[diagnostic_name]
                )
        ...
        dataset.attrs["generation_complete"] = True
